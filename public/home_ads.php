<?php
// =========================================================================
// PHẦN LOGIC KHỞI TẠO DỮ LIỆU (MOCK DATA)
// Mục đích: Định nghĩa các biến để View bên dưới không bị lỗi Undefined
// Sau này bạn sẽ thay phần này bằng Code lấy dữ liệu từ Database thật.
// =========================================================================

// 1. Giả lập thông tin User (Nếu chưa login thì để null)
// $currentUser = ['id' => 1, 'username' => 'admin', 'coin' => 500000]; // Uncomment dòng này để test giao diện đã login
$currentUser = isset($_SESSION['user']) ? $_SESSION['user'] : null; // Lấy từ session thật nếu có

// 2. Cấu hình số lượng Slot tối đa cho từng vị trí
$maxSlots = [
    'LEFT_SIDEBAR'  => 5,
    'RIGHT_SIDEBAR' => 5,
    'HERO'          => 1, // VIP Banner Giữa
    'STD'           => 10 // Banner Giữa Nhỏ
];

// 3. Giả lập số lượng đã đặt (Query DB: SELECT count(*) ...)
$qtyInfo = [
    'LEFT_SIDEBAR'  => 2,  // Đã đặt 2/5
    'RIGHT_SIDEBAR' => 0,  // Đã đặt 0/5
    'HERO'          => 1,  // Đã đặt 1/1 (Full)
    'STD'           => 4   // Đã đặt 4/10
];
...
$prices = [
    'STD' => 50000,
    'LEFT_SIDEBAR' => 100000,
    'RIGHT_SIDEBAR' => 100000,
    'HERO' => 500000
];

// 5. Tính toán logic trạng thái (Availability & Boolean Flags)
$availability = [];
$nextAvailableMap = []; // Ngày giờ trống tiếp theo

// --- Logic Trái ---
$isFullLeft = ($qtyInfo['LEFT_SIDEBAR'] >= $maxSlots['LEFT_SIDEBAR']);
$availability['LEFT_SIDEBAR'] = $isFullLeft ? "ĐÃ ĐẦY" : "SẴN SÀNG";
if ($isFullLeft) $nextAvailableMap['LEFT_SIDEBAR'] = date('c', strtotime('+2 days')); // Giả lập 2 ngày nữa hết hạn

// --- Logic Phải ---
$isFullRight = ($qtyInfo['RIGHT_SIDEBAR'] >= $maxSlots['RIGHT_SIDEBAR']);
$availability['RIGHT_SIDEBAR'] = $isFullRight ? "ĐÃ ĐẦY" : "SẴN SÀNG";
if ($isFullRight) $nextAvailableMap['RIGHT_SIDEBAR'] = date('c', strtotime('+5 days'));

// --- Logic Hero (VIP) ---
$isFullHero = ($qtyInfo['HERO'] >= $maxSlots['HERO']);
$availability['HERO'] = $isFullHero ? "ĐÃ ĐẦY" : "ĐANG TRỐNG"; // VIP dùng từ khác chút cho sang
if ($isFullHero) $nextAvailableMap['HERO'] = date('c', strtotime('+10 days'));

// --- Logic Standard (Giữa nhỏ) ---
$isFullStd = ($qtyInfo['STD'] >= $maxSlots['STD']);
$availability['STD'] = $isFullStd ? "HẾT SLOT" : "CÒN SLOT";
if ($isFullStd) $nextAvailableMap['STD'] = date('c', strtotime('+1 day'));

// =========================================================================
// KẾT THÚC PHẦN LOGIC - BÊN DƯỚI LÀ GIAO DIỆN HTML
// =========================================================================
?>
